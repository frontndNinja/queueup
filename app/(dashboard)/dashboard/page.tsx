...
import { getMemberQueues, getOwnedQueues } from "@/actions/queues";
import {
    Carousel,
    CarouselContent,
    CarouselItem,
...
export default async function DashboardPage() {
    ...
    const memberQueues = await getMemberQueues();
    //const ownerQueues = await getOwnedQueues();

    return (
        <div className="min-h-screen">
            <section className="w-full h-full flex gap-4 flex-col p-4">
                <h1 className="sm:text-lg-p1 mt-8 mb-4 px-6">Welcome back to QueueUp, {user.name}!</h1>
...
export default async function DashboardPage() {
    ...
    return (
        <div className="min-h-screen">
            <section className="w-full h-full flex gap-4 flex-col p-4">
                <h1 className="sm:text-lg-p1 mt-8 mb-4 px-6">
                ...
                {safeQueues.length === 0 ? (
                    ...
                    safeQueues.map((queue) => (
                        <div
                            key={queue.id}
                            className="bg-[linear-gradient(90deg,rgba(0,0,0,0)_0%,rgba(27,36,48,0.1)_15%,rgba(27,36,48,0.2)_30%,rgba(27,36,48,0.2)_50%,rgba(27,36,48,0.2)_70%,rgba(27,36,48,0.1)_85%,rgba(0,0,0,0)_100%)] text-card-foreground px-6 py-4 pb-6 mb-6 rounded-sm"
                        >
                            <div className="max-w-[1440px] m-auto">
                                <Link href={`/dashboard/queue/${queue.id}`} className="inline-flex">
